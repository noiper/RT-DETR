...
def main():
    ...
    def _run_key_frame(img_key, target_key, step_idx):
        nonlocal latest_key_results, latest_key_results_norm, out_k
        ...
        if args.baseline:
            norm_sizes = torch.ones_like(orig_sizes_k, device=device)
            latest_key_results_norm = postprocessor(out_k, norm_sizes)

    def _run_non_key_frame(img_key, target_key, step_idx):
        target_loss = prepare_targets_for_loss(target_key, device)
        target_key = _move_targets(target_key)
        if args.baseline:
            ...
            res_nk_batch = []
            for key_result_norm, target in zip(latest_key_results_norm, target_key):
        ...
        orig_sizes_nk_diag = torch.stack([t["orig_size"] for t in target_key], dim=0).to(device)
        for result, target, orig_size in zip(res_nk_batch, target_key, orig_sizes_nk_diag):
        ...
        for batch in tqdm(val_dataloader, desc="Streaming Video"):
            ...
            for sample_pos in range(batch_size):
                ...
                if is_key_frame:
                    ...
                    _run_non_key_frame(img_key, target_key, step_idx)
